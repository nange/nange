		...
		- **Parquet** 没有null bitmap，其使用“Repetition Levels”和“Definition Levels”共同确定数据的层级和重复列表以及是否为Null。
			...
			- 一个更完整的例子：同时包含定义级别(Definition Levels)和重复级别(Repetition Levels)
				...
				- 在这个例子中，"a" 是一个包含列表的列表的列表。这个数据在 Parquet 文件中的表示可能如下：
					- ```
					  Definition Levels: 3, 3, 3, 3, 3, 3, 3
					  Repetition Levels: 0, 2, 1, 2, 0, 2, 2
					  Values:            1, 2, 3, 4, 5, 6, 7
					  ```
				- 在这个例子中，所有的元素都存在并且不是 null，所以所有的定义级别都是 3。
				- 然而，重复级别的值取决于元素在数据结构中的位置。例如，第一个元素 "1" 是第一个记录的 "a" 字段的第一个列表的第一个列表的第一个元素，所以它的重复级别是 0；
				- 第二个元素 "2" 是第一个记录的 "a" 字段的第一个列表的第一个列表的连续元素，所以它的重复级别是 2；
				- 第三个元素 "3" 是第一个记录的 "a" 字段的第一个列表的新的列表的第一个元素，所以它的重复级别是 1；
				- 第四个元素 "4" 是第一个记录的 "a" 字段的新的列表的第一个列表的第一个元素，所以它的重复级别是 2；以此类推。